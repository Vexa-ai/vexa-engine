...
from app.utils.db import get_session, async_session
...
async def can_access_transcript(session: AsyncSession, user_id: UUID, content_id: UUID) -> bool:
    ...
    return access in [AccessLevel.VIEWER, AccessLevel.EDITOR, AccessLevel.OWNER]
...
async def restore_content(
    ...
    user_id: UUID,
    content_id: UUID,
    restore_access_level: AccessLevel = AccessLevel.OWNER
) -> bool:
...
async def get_content_with_access(
    session: AsyncSession,
    user_id: UUID,
    content_id: UUID,
    required_access: AccessLevel = AccessLevel.VIEWER
) -> Tuple[Content, UserContent]:
    """Get content with access validation"""
    result = await session.execute(
    ...
    if AccessLevel(user_content.access_level).value < required_access.value:
