...
from app.models.schema.search import ContentResponse
...
class HybridSearchEngine:
    ...
    async def search(self, query: str, limit: int = 10) -> List[ContentResponse]:
        # Get results from both engines
        qdrant_results = await self.qdrant_engine.search(query, limit=limit)
        es_results = await self.es_engine.search(query, limit=limit)
        
        # Combine and deduplicate results
        seen_ids = set()
        combined_results = []
        
        # Interleave results from both engines
        for i in range(max(len(qdrant_results), len(es_results))):
            if i < len(qdrant_results):
                result = qdrant_results[i]
